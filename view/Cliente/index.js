...
function editarCliente(id) {
    ...
    $("#cliente_id").val(id);
    $('#modalClienteLabel').html('Editar cliente');
    
    // Ocultar card de dirección principal (ya tiene direcciones registradas)
    $('#card_direccion_principal').hide();

    $.post("../../controller/cliente.php?op=obtener", { id : id }, function (data) {
        ...
        if(data.estado == '2'){
            $('#estado_cliente').prop('checked', false); 
        } else {
...
function estadoCliente(cliente_id, pAccion) {
    let estado = pAccion == 1 ? 'activar' : 'inactivar';

    Swal.fire({
        text: `¿Deseas ${estado} el cliente?`,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '../../controller/cliente.php?op=estado_cliente',
                type: 'POST',
                dataType: 'json',
                data: {
